...
py vorlage-abgleich.py ..\healthliteracy-portal --uebernehmen
```
...
| Live prüfen | `curl -s "https://healthliteracy.m-vf.de/?cb=$(date +%s)"` — Cache-Buster nötig |
| Pages-Status | `gh api repos/mvf-portal/healthliteracy-portal/pages/builds/latest` |
...
|---|---|---|
| `HLHUB` | Claude-API für die Studienauswahl | Der tägliche Lauf bricht ab, `index.html` bleibt unverändert |
| `HLHUBMC` | Mailchimp-API für den Kampagnen-Entwurf | Nur dieser Schritt entfällt (`continue-on-error`) |
...
**Alle Portale schreiben in dasselbe Mailchimp-Konto.** `mailchimp_entwurf.py` erkennt seine eigenen Kampagnen am Titel-Präfix `MVF Gesundheitskompetenz-Newsletter`. Der Präfix muss sich **vollständig** von den Schwesterportalen unterscheiden: `datum_aus_titel()` prüft mit `startswith()`, ein bloßer Zusatz reicht nicht.

Mailchimp-Gruppe dieses Portals: `group[16139][8192]` („Studien Newsletter HealthLiteracy"). **Gruppen-Nummern sind Identitäten, keine Beschriftungen** — wer eine umbenennt, verschiebt Menschen, nicht Wörter.

Die Anmeldeseite `newsletter.html` kennt diese Nummer seit dem 01.09.2026 nicht mehr. Sie sendet an den Anmelde-Endpunkt auf dem MVF-Server (`/anmeldung/anmeldung.php`, beschrieben in `mvf-server/anmeldung/LIESMICH.md` im Repo `knowledge-hubs`) und nennt dort nur den Schlüssel dieses Hubs: `healthliteracy`. Welche Gruppe dahintersteht, weiß allein der Server — **ein Schlüssel, der nicht in seiner Zulassungsliste steht, wird abgewiesen.** Ein neuer Hub braucht deshalb drei Eintragungen in dieser Reihenfolge: Gruppe in Mailchimp, Kennung in `anmeldung-zugang.php`, Schlüssel hier.
